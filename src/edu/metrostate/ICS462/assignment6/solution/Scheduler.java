package edu.metrostate.ICS462.assignment6.solution;
...
public abstract class Scheduler extends Thread {
	...
	public int sleep(int tracks) {
		...
		try {
			int time = Math.max(1, tracks * edu.metrostate.ICS462.assignment6.part3.Analyzer.SWEEP_TIME / edu.metrostate.ICS462.assignment6.part3.Analyzer.NUMBER_OF_CYLINDERS);
			Thread.sleep(time);
			return time;
	...
	public void printStatistics(String algorithm) {
		...
		System.out.println("Number of tracks moved " + traveledDistance);
		System.out.println("Requests Processed " + processed + " time spent " + time);
